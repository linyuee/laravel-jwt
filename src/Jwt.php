...
class Jwt
{
    ...
    private $payload;
    private $token_ttl;
    private $refresh_token_ttl;

    public function __construct()
    {
        $this->token_ttl = config('jwt.token_ttl');
        $this->refresh_token_ttl = config('jwt.refresh_token_ttl');
        $this->key = config('jwt.key');
        $this->header['alg'] = empty(config('jwt.alg'))?'HS256':config('jwt.alg');
    }
     ...
     * @return bool|string
     */
    public function getRefreshToken(array $payload){
        if (is_array($payload)) {
            $payload = array_merge([
                'iss' => 'local', //该JWT的签发者
                'iat' => time(), //签发时间
                'exp' => time() + $this->refresh_token_ttl, //过期时间
                'nbf' => time(), //该时间之前不接收处理该Token
                'sub' => 'local', //面向的用户
                'jti' => md5(uniqid('JWT') . time()) //该Token唯一标识
            ], $payload);
    ...
    public function getPayload($key = null){
        return empty($key) ? $this->payload : $this->payload[$key];
    }
    ...
    public function set($key,$value){
        ...
        $ttl = $this->payload['exp'] - time();
        \Cache::put($this->payload['jti'] . $key,$value,$ttl/60);
    }

    public function get($key){
        if (empty($this->payload))return false;
        return \Cache::get($this->payload['jti'] . $key);
    }
